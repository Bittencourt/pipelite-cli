...
| Grep pins | `--description` absent from cli/webhooks.rs and api-reference webhooks section (amendment) · "secret" absent from cache.rs · `Validation` absent from purge.rs · `--all` absent from cli/audit.rs · normalize called by all three trash subcommands · forbidden_hint table untouched (hints pre-registered Phase 8) |
