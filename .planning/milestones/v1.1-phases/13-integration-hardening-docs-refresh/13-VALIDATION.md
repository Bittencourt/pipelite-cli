...
| SC-1 (formats) | csv header row derived from each group's default_columns (runs/templates/notes/webhooks/custom-fields contain "id"; trash/audit do NOT — models.rs:961/:1001), ≥2 lines, + plain non-empty output for all 7 list-bearing new groups (runs, templates, notes, webhooks, trash, audit, custom-fields); batch/docs exempt with table comments | same (13-01 T2) | csv_and_plain rows (7×2) | same | — per-group default_columns-derived headers (trash/audit have NO id column)
