...
fn collect_ids(args: &PeopleDeleteArgs) -> Result<Vec<String>> {
    if args.stdin {
        ...
        if io::stdin().is_terminal() {
            return Err(CliError::Validation {
                ...
                hint: "Pipe JSON data: echo '[\"per_1\",\"per_2\"]' | pipelite people delete --stdin"
                    .to_string(),
            }
            .into());
        }
...
/// Batch delete people with confirmation prompt (per D-04, D-05, D-06).
async fn batch_delete(ctx: &AppContext, ids: &[String]) -> Result<()> {
    ...
    if ctx.dry_run {
        for id in ids {
            let url = format!("{}/api/v1/people/{}", ctx.client.base_url(), id);
            dry_run::render_dry_run_delete("person", id, &url, &ctx.output_format, ctx.color)?;
        }
        return Ok(());
    }
    ...
    if !ctx.no_input && io::stdin().is_terminal() {
        let confirm = dialoguer::Confirm::new()
            .with_prompt(format!("Delete {} person(s)?", total))
            .default(false)
            .interact()?;
        if !confirm {
            return Ok(());
    ...
    for (i, id) in ids.iter().enumerate() {
        match ctx.client.delete_person(id).await {
            Ok(()) => {
    ...
    outcome.finalize("person", "delete")
}
