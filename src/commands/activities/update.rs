...
async fn batch_update(ctx: &AppContext) -> Result<()> {
    if io::stdin().is_terminal() {
        return Err(CliError::Validation {
            ...
            hint: "Pipe JSON data: echo '[{\"id\":\"act_1\",\"title\":\"New\"}]' | pipelite activities update --stdin".to_string(),
        }
        .into());
    }
    ...
    for (i, item) in items.into_iter().enumerate() {
        ...
        match ctx.client.update_activity(&id, &data).await {
            Ok(activity) => {
                outcome.record_success();
                succeeded.push(activity);
            }
            Err(e) => {
                outcome.record_failure(i, &id, &e);
            }
    ...
    outcome.finalize("activity", "update")
}
