...
fn collect_ids(args: &OrgsDeleteArgs) -> Result<Vec<String>> {
    if args.stdin {
        ...
        if io::stdin().is_terminal() {
            return Err(CliError::Validation {
                ...
                hint: "Pipe JSON data: echo '[\"org_1\",\"org_2\"]' | pipelite orgs delete --stdin"
                    .to_string(),
            }
            .into());
        }
...
/// Batch delete organizations with confirmation prompt (per D-04, D-05, D-06).
async fn batch_delete(ctx: &AppContext, ids: &[String]) -> Result<()> {
    ...
    if ctx.dry_run {
        for id in ids {
            let url = format!("{}/api/v1/organizations/{}", ctx.client.base_url(), id);
            dry_run::render_dry_run_delete("organization", id, &url, &ctx.output_format, ctx.color)?;
        }
        return Ok(());
    }
    ...
    if !ctx.no_input && io::stdin().is_terminal() {
        let confirm = dialoguer::Confirm::new()
            .with_prompt(format!("Delete {} organization(s)?", total))
            .default(false)
            .interact()?;
        if !confirm {
            return Ok(());
    ...
    for (i, id) in ids.iter().enumerate() {
        match ctx.client.delete_org(id).await {
            Ok(()) => {
    ...
    outcome.finalize("organization", "delete")
}
